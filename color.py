...
def to_rgb01(arr):
    """任意ビット深度RGB → float32 [0,1]（数値の意味を保つ。プレビュー用の強調は別処理）"""
...
def percent_stretch_u8(x, p_lo=2, p_hi=98):
    """プレビュー専用：float32 → パーセンタイルで 0..255 に正規化"""
...
def rgb_to_xyz_real(rgb01):   # おおむね [0,1]（若干の越境はあり得る）
    return cv2.cvtColor(rgb01, cv2.COLOR_RGB2XYZ).astype(np.float32)

def rgb_to_luv_real(rgb01):   # L[0..100]、u,v は負値あり得る
...
# ---- B: JPEG/PNG ----
    ...
    st.subheader("🧭 JPEG/PNG（プレビュー用途。数値解析は GeoTIFF を使用）")
    try:
        ...
        # GeoTIFF（float32、座標不明時は単位ピクセル変換）
        ...
        # PNG/JPEG はプレビュー用（EXIF は可能ならコピー）
        png_io = io.BytesIO(); Image.fromarray(prev_u8).save(png_io, format="PNG", compress_level=6)
        st.download_button("⬇️ PNG（プレビュー）", data=png_io.getvalue(),
