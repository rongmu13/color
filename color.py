...
with st.sidebar:
    ...
    enable_8bit_scale = st.checkbox(
        "高ビット深度画像を8bitへスケーリングしてから変換（LAB実数には無関係）",
        value=True,
        help="ドローン GeoTIFF は 10/12/16bit が多いので、8bit 変換が安定します。"
    )
...
def rgb_to_lab_real_opencv(rgb_any_dtype):
    """返回 float32：L[0..100]，a/b≈[-127,127]"""
    ...
    try:
        with rasterio.MemoryFile(uploaded.read()) as mem:
            with mem.open() as src:
                ...
                st.write(f"原画像サイズ：{w} × {h}／バンド数：{count}（先頭{bands_to_read}バンド使用）")
                ...
                with out_mem.open() as prev_ds:
                    ...
                    if color_space == "LAB" and lab_real:
                        if preview.dtype != np.float32: preview = preview.astype(np.float32)
                ...
                st.download_button("⬇️ 変換結果をダウンロード（GeoTIFF）",
                                   data=out_bytes, file_name=out_name, mime="image/tiff")

        st.success("✅ 変換完了。GeoTIFF の投影・座標・タグは可能な範囲で引き継ぎ済み。")
    ...
    try:
        ...
        if lab_geotiff_bytes is not None:
            ...
            st.download_button("⬇️ GeoTIFF（実数LAB: float32）をダウンロード",
                               data=lab_geotiff_bytes, file_name=gt_name, mime="image/tiff")

        png_io = io.BytesIO()
        Image.fromarray(prev_u8).save(png_io, format="PNG", compress_level=6)
        st.download_button("⬇️ PNG をダウンロード（可視化）",
                           data=png_io.getvalue(),
                           file_name=Path(filename).stem + f"_{color_space}.png",
                           mime="image/png")
        ...
        if is_jpeg: btn += " ✅EXIFコピー済" if exif_ok else " ⚠EXIFコピー不可"
        st.download_button(btn, data=jpg_io.getvalue(),
                           file_name=Path(filename).stem + f"_{color_space}.jpg",
                           mime="image/jpeg")
